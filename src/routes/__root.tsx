...
function ErrorComponent({ error, reset }: { error: Error; reset: () => void }) {
  ...
  useEffect(() => {
    // Expired session errors fix themselves: silent refresh first, then
    // sign out + back to sign-in only if the refresh fails.
    if (authExpired) {
  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Mehar DVR — Daily Visit Report" },
      {
        ...
        content:
          "GPS-verified field visit reporting with live photo capture for Mehar Advisory teams.",
      },
...
function RootComponent() {
  ...
  useEffect(() => {
    const ephemeral = localStorage.getItem("mehar_ephemeral");
    if (ephemeral && !sessionStorage.getItem("mehar_alive")) {
  ...
  useEffect(() => {
    const handleStorage = (e: StorageEvent) => {
      if (e.key === "dvr_token" || e.key === "token") {
        router.invalidate();
